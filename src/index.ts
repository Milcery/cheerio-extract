...
class CheerioExtract {
  ...
  useFilter = (filterName: string, func: any) => {
  ...
  query = (rule: string = '', dom: any = undefined) : any  => {
    return parseRule(rule).reduce(this.parse, dom || this.$('body'));
  }
  ...
  filterFn = {
    html: (dom: CheerioAPI) => dom.html(),
    text: (dom: CheerioAPI) => dom.text(),
